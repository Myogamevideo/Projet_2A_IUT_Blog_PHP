<?php
function getConnexion($db, $postpseudo, $postpassword, $postcase, $cookiepseudo, $cookiepassword, $cookiestatu)
{
    if (isset($cookiepseudo) && isset($cookiepassword) && isset($cookiestatu)) {
        ...
        //$req->execute(array($cookiepseudo));
        $donne = $req->fetch();
        if ($cookiepassword == $donne['pass']) {
            //$req = $bdd->prepare('select id,statu from membres where pseudo=?');
            //$req->execute(array($cookiepseudo));
            //$donne = $req->fetch();
        ...
        if ($donne['nbr'] != 0) {
            ...
            if ($verify == true) {
                ...
                if ($postcase == 'on') {
                    ...
                    $req = $db->query($query, array(':pseudo' => $postpseudo));
                    //$req = $bdd->prepare('select pass,statu from membres where pseudo=?');
                    //$req->execute(array($postpseudo));
                    $donne = $req->fetch();
        ...
        if ($donne['nbr'] != 0) {
            ...
            if ($postconfig_password != $postpassword) {
                ...
                if (preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $postemail)) {
                    ...
                    $req = 'INSERT INTO membres (pseudo,pass,email,date_inscription,statu) VALUES (:pseudo,:pass,:email,now(),:statu)';
                    $req = $db->query($query, array(
                        ':pseudo' => $postpseudo,
                        ':pass' => $pass_hach,
                        ':email' => $postemail,
                        ':statu'  => 'null'));
                    //$req = $bdd->prepare('insert into membres (pseudo,pass,email,date_inscription,statu) values (:pseudo,:pass,:email,now(),:statu)');
                    /*$req->execute(array(
                        'pseudo' => $postpseudo,
                        'pass' => $pass_hach,
                        'email' => $postemail,
